-- Migration 038: Donation log table and generous_giver badge eligibility.
-- Story 13.3: Spring Clean Declutter Flow & Donations.
-- FR-DON-01, FR-DON-02, FR-DON-03, FR-DON-04, FR-DON-05, FR-HLT-05

-- ============================================================
-- 1. donation_log table
-- ============================================================
CREATE TABLE app_public.donation_log (
  ...
  charity_name TEXT DEFAULT NULL,
...
-- Enable RLS
ALTER TABLE app_public.donation_log ENABLE ROW LEVEL SECURITY;

-- RLS policy: users can only access their own donation log entries
CREATE POLICY donation_log_user_policy ON app_public.donation_log
  FOR ALL
  USING (profile_id IN (
...
-- Indexes
CREATE INDEX idx_donation_log_profile ON app_public.donation_log(profile_id, created_at DESC);
CREATE INDEX idx_donation_log_item ON app_public.donation_log(item_id);

-- ============================================================
-- 2. Update evaluate_badges function: replace generous_giver placeholder
-- ============================================================
  ...
  LOOP
    ...
    CASE badge_key
      ...
      WHEN 'generous_giver' THEN
        SELECT COUNT(*) >= 20 INTO v_earned
        FROM app_public.donation_log WHERE profile_id = p_profile_id;
